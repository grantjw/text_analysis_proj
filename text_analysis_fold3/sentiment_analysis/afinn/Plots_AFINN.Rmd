...
# China vs. Non-China
```{r}
load("sentiment_df_AFINN")

library(ggplot2)
library(dplyr)

df2 <- df1 %>% mutate(china =
                     case_when(issue65 == 0 ~ "Non-China Ads", 
                               issue65 == 1 ~ "China Ads"))

# Boxplots
p1 <- df2 %>% 
  filter(!is.na(china)) %>% 
  ggplot(aes(x=china, y=difference, fill = issue65)) +
  geom_boxplot() 
p1 + theme_classic()
...
t.test(df3$difference[df3$issue65=="0"], df3$difference[df3$issue65=="1"])
```
...
t.test(df3$difference[df3$issue65=="0" & df3$party=="Republican"], 
       df3$difference[df3$issue65=="1" & df3$party=="Republican"])

t.test(df3$difference[df3$issue65=="0" & df3$party=="Democrat"], 
       df3$difference[df3$issue65=="1" & df3$party=="Democrat"])
...
```{r}
p1 <- ggplot(df1, aes(x=issue65, y=difference, fill = issue20)) +
  geom_boxplot()
...
```{r}
p1 <- df1 %>%
      filter(issue65 == 1 | issue65 ==0) %>% 
      ggplot(aes(x=year, y=difference, fill = issue65)) +
      geom_boxplot()
p1 + 
  theme_classic() 
```
...
df1 <- df1 %>% mutate(china_trade =
                     case_when(issue65 == 0 & issue20 == 0 ~ "NC_NT", 
                               issue65 == 1 & issue20 == 0 ~ "C_NT",
                               issue65 == 0 & issue20 == 1 ~ "NC_T",
                               issue65 == 1 & issue20 == 1 ~ "C_T")
)
...
df1 <- df1 %>% mutate(china_trade_party =
                     case_when(issue65 == 0 & party == 1 & issue20 == 1 ~ "NC_D_T", 
                               issue65 == 1 & party == 1 & issue20 == 1 ~ "C_D_T",
                               issue65 == 0 & party == 2 & issue20 == 1 ~ "NC_R_T",
                               issue65 == 1 & party == 2 & issue20 == 1 ~ "C_R_T",
                               issue65 == 0 & party == 1 & issue20 == 0 ~ "NC_D_NT",
                               issue65 == 1 & party == 1 & issue20 == 0 ~ "C_D_NT",
                               issue65 == 0 & party == 2 & issue20 == 0 ~ "NC_R_NT",
                               issue65 == 1 & party == 2 & issue20 == 0 ~ "C_R_NT",
                               ))
...
```{r}
df_points_1 <- df1 %>% group_by(year, issue65,party) %>%
  #filter(party == 2) %>%
...
p1 <- df_points_1 %>%
      ...
      geom_point(aes(shape = issue65, color = party), size =3) +
...
```{r}
df_points_1 <- df1 %>% group_by(electype, issue65,party) %>%
  #filter(party == 2) %>%
  dplyr::summarise(Sent_Difference = mean(difference, na.rm = FALSE))
...
```{r}
write.csv(df1, file ="sentiment_df.csv" )
save(df1, file = "sentiment_df.Rdata")
