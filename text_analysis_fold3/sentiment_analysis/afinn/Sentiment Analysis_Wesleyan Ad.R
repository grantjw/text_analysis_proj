...
df1 %>% group_by(issue65) %>% dplyr::summarise(Sent_Ratio= mean(ratio, na.rm = FALSE))
df1 %>% group_by(issue65) %>% dplyr::summarise(Sent_Difference= mean(difference, na.rm = FALSE))

summary(m1 <- lm(ratio ~ issue65, data=df1))
...
df1 %>% group_by(issue65, issue20) %>% dplyr::summarise(Sent_Ratio = mean(ratio, na.rm = FALSE))
df1 %>% group_by(issue65, issue20) %>% dplyr::summarise(Sent_Difference = mean(difference, na.rm = FALSE))

summary(m1 <- lm(ratio ~ issue20, data=df1, subset=issue65==1))
...
summary(m1 <- lm(ratio ~ party, data=df1, subset=issue65==1))
...
df1 %>% group_by(electype, issue65) %>% dplyr::summarise(Sent_Ratio= mean(ratio, na.rm = FALSE))
df1 %>% group_by(electype, issue65) %>% dplyr::summarise(Sent_Difference= mean(difference, na.rm = FALSE))

summary(m1 <- lm(ratio ~ issue65, data=df1, subset=electype=="gov"))
summary(m1 <- lm(difference ~ issue65, data=df1, subset=electype=="gov")) # p=0.053
summary(m1 <- lm(ratio ~ issue65, data=df1, subset=electype=="house")) # p=0.009
summary(m1 <- lm(difference ~ issue65, data=df1, subset=electype=="house")) # p=0.053
summary(m1 <- lm(ratio ~ issue65, data=df1, subset=electype=="senate"))
summary(m1 <- lm(difference ~ issue65, data=df1, subset=electype=="senate"))
summary(m1 <- lm(ratio ~ issue65, data=df1, subset=electype=="pres"))
